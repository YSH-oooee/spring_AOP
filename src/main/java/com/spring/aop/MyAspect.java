...
- 이 부가(공통)기능들은 프로젝트에 굉장히 중요한 역할을 하며,
  이 부가(공통)기능이 코드마다 반복적(중복)으로 나타나는 부분이 존재한다. 
- 코드에서 비즈니스 핵심 로직과 부가기능을 분리하여 부가 로직을 따로 관리(모듈화)한다.
- 종단(비즈니스 로직) 기능 , 횡단(관심,Aspect) 기능 
- 부가 기능이 비즈니스 로직(핵심 기능)을 담은 클래스의 코드에 전혀 영향을 주지 않으면서
  부가기능의 구현을 용이하게 할 수 있는 구조를 제공한다.
...
Pointcut   : Aspect 적용 위치 지정자      ( advice를 어디에 적용할지를 결정=중복되는 위치를 pointCut으로 따로 빼줌 )
...
@Aspect	//AOP에서 Aspect 어노테이션을 사용해주어야 기능이 활성화 됨
public class MyAspect {
	...
	//메소드 호출 전
//	@Before("execution(public void com.spring.aop.ClassBoss.work())")	//특정패키지.특정클래스.특정메소드 지정
//	@Before("execution(* work())")	//모든 파일의 work() 메소드 전에 실행
	@Before("pointCut()")
	public void before() {
...
//		System.out.println("AOP After 메소드 호출 : 퇴근한다.\n");
	...
	//메소드 호출 전+후
//	@Around("execution(* getWorkTime())")	//* getWorkTime() = pjp
	...
	public void around(ProceedingJoinPoint pjp) {
		...
		//메소드 호출
		try {
			pjp.proceed();		//ProceedingJoinPoint의 proceed 메소드를 호출
		} catch (Throwable e) {
			e.printStackTrace();
		}
	...
	//호출된 메소드가 성공적으로 실행된 후, JoinPoint를 통하여 메소드의 파라미터를 전달받음
	...
	public void afterReturning(JoinPoint jp) {
		...
		System.out.println("AOP AfterRturning 메소드 호출 \n\n");
	...
	public void afterThrowing() {
		System.out.println("AOP AfterThrowing 메소드 호출\n");
		//afterThrowing()이 실행(출력)된 후, 에러 메세지 출력
	}
